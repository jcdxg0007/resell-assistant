...
REM   phone_4.env.local.bat —— git pull 永不覆盖（roadmap §15.4）。
REM 首次部署：复制 phone.env.example.bat 为 phone_4.env.local.bat 再填真实值。
REM ============================================================
...
if not exist "%~dp0phone_4.env.local.bat" (
  echo [错误] 缺少 phone_4.env.local.bat —— 请复制 phone.env.example.bat 改名并填真实值
  pause
  exit /b 1
)
call "%~dp0phone_4.env.local.bat"
