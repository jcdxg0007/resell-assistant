...
`seen_count ≥ 5` 时自动 pin（标记为 `pinned_at=NOW()`），保护其不被每日清库流程误删。这样系统会自动识别"长期热品"，不需要全部依赖人工 pin。

### 10.3 每日清库（Phase 1 Day 5 实现）

每天凌晨 3 点（活动窗口外）跑 celery beat：
- 删除：`pinned_at IS NULL AND last_seen_at < today - 24h`
- 保留：所有 pinned（人工 + 自动 pin）
- 保留：评分 > 阈值 且 last_seen_at > today - 7d 的潜在好货
- 删除前发钉钉简报："清了 N 条 / 保留 M 条（其中 pin K 条）"
