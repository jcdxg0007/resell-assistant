...
| 「查物流」拟人行为档位 | ✅ v1 已上线（每日探测+冷却+前端开关，selector 待真机校准；见 §11.4） |
...
- **触发点**：`main._handle_search` 里 burst 结束（`burst_continues=False`）**且本次搜索没报错/没被风控**时，在退 PDD 前调 `cli.maybe_browse_logistics()`。按概率（默认 0.25，前端可调）触发，best-effort 异常 swallow。
- **每日探测 + 冷却**（用户 2026-06-05 拍板）：状态机在 `pdd_app_client` 模块级维护（一个 worker 进程=一个采集号），`unknown / has_orders / cooldown` 三态，按东八区 day key 跨天自动重置。
  - 当天**首次触发**：进 个人中心 → 我的订单，识别订单页。**有真实订单** → 标 `has_orders`，当日后续触发继续随机查；**订单页为空** → 标 `cooldown`，当日不再尝试（空订单页反而是异常信号）；**导航没到订单页** → 保持 `unknown`，下次再试（不误判为冷却）。
- **路径**：个人中心 → 我的订单 →（识别到「查看物流/确认收货/评价/再次购买」等按钮=有单）→ 点「查看物流」→ 停留 ~1.5-3s + 滑 1-2 下 → press back 2-3 次返回 → 由 cleanup 的 home press 收尾。轻量版（不逐个点开包裹）。
...
- **⚠️ 前提**：该号要**真有历史订单**——没有就会命中"当日冷却"，开了也不会乱逛空订单页。建议只对有真实购买记录的号开。
- **⚠️ 待办**：订单页/物流页的 selector 是**初稿**（多 selector 兜底），需真机 `uiautomator2 inspect` 校准；PDD 这些页改版后要在 `pdd_app_client._browse_logistics_once` 更新 xpath。
