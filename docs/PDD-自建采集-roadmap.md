...
| 「查物流」拟人行为档位 | ✅ v1 已上线（静默期 B 触发 + 每日探测/冷却 + OCR 兜底 selector + 前端开关；真机 2026-06-05 验证通过；见 §11.4） |
...
- **触发点（B 方案，2026-06-05 真机验证后切换）**：从"每个 burst 结尾"(A) 挪到 **inter-burst 静默期中段**触发，更像真人"两波搜索之间点亮手机看眼快递"，而非"每搜完一波立刻查一遍再放下"。
  - 实现：`BurstScheduler._quiet_sleep_with_break` 把整段静默 sleep 切成「睡一会 → 插一次查物流 → 睡剩下」，插入点在静默 [25%, 75%] 区间随机（避免固定相位指纹），插入动作耗时从剩余里扣掉、保证总静默 ≈ 目标 gap（不破坏 burst 节奏画像）。静默 < 90s 时退化为整段 sleep。
  - 回调 `main._quiet_logistics_break`：先调**廉价门控** `pdd_app_client.should_browse_logistics()`（开关/冷却/概率，**不碰设备**），命中才前台化 PDD 跑 `cli.browse_logistics_now()`，结束 press home 退后台。避免"概率没中却白白开 PDD"。
  - 入口拆分：`maybe_browse_logistics()` = `should_browse_logistics()` 门控 + `browse_logistics_now()` 执行；静默期走"先门控再执行"两段式。
...
- **开关**：`pdd_worker_config` 加 `logistics_browse_enabled`（默认 **False**）/ `logistics_browse_prob`（默认 0.25），走 runtime-config 热更新（`apply_remote_config → set_logistics_browse`，≤45s 生效）。前端「多平台比价」页 PDD 全自动采集卡片里有「查物流」开关 + 概率输入。
