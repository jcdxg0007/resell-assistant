...
### 11.4 「查物流」拟人行为（2026-06-02 讨论定，待实现）

**动机**：当前 `_idle_browse_warmup` 只会"看推荐流 / 看商品详情"，行为太单一（本质=搜了又搜+看商品）。
真人会查订单/物流，加这个提升**行为多样性**——正是 PDD 风控关注的维度。

- **触发**：倾向**每个 burst 结束时小概率（20~30%）触发一次**，或每 N 个 burst 触发一次。放结束而非开始，避免拖慢首个搜索。
- **路径**：底部「个人中心」tab → 我的订单 → 点一个已完成订单 → 查看物流 → 停留 1.5~3s + 滑几下 → 返回首页。先做**轻量版**（扫一眼物流就返回，不逐个点开包裹）。失败 swallow，不影响主搜索。
- **⚠️ 前提**：该号要**真有历史订单**，否则订单页为空反而是异常信号 → 绑定到有真实购买记录的号。
- **成本**：要新写订单页/物流页定位（PDD 这些页也会改版），维护量比纯列表大；做成 warmup 的一个新档位。
