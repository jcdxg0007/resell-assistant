...
| 详情页补抓字段 | — | SKU / 评论数 / 历史价 / 店铺评分 / 完整描述 / 百亿补贴真实价(OCR) |
...
1. **候选打分**：列表抓完后，对每条按 `标签数(badges) + 销量` 排序（销量缺失记 0；
   ...
   出于风控；具体值实现时再定，可做成配置）。
2. **进详情页补抓**：对 Top-K 逐条点进商品详情页，抓 SKU / 评论数 / 历史价 /
   店铺评分 / 完整描述；**百亿补贴真实价仍要走 OCR**（canvas 自绘，u2 抓不到，见 Day 4）。
