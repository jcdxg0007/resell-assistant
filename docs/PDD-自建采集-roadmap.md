...
### 10.3 数据保留与每日清库（2026-06-06 重写：从「每天清空」改为「按 N 天保留」）

> **演进**：早期是"每天凌晨清空、当日重新来过"。问题：① 0 点逻辑日界与 3 点物理
> 清库错位，造成 0-3 点"假清库"空窗；② 选品/比价只能看当天数据，跨天趋势、昨天
> 采到的词全丢。现已统一为**逻辑日界 = 物理清库 = 凌晨 3 点（东八区）**，并把"清空"
> 换成**可配置的 N 天保留**。

**逻辑日界（`_cn_day_start`）= 每天 03:00 东八区**：去重（"每词每天只跑一遍"）、
显示口径、清库 cutoff 全部对齐这一点，消除 0 点假清库空窗（commit `71277f5`）。

**Celery beat 凌晨清理链（错峰，活动窗口外）**：

| 时刻 | 任务 | 清什么 | 保留天数（运行时配置 → 默认） |
|---|---|---|---|
| 03:00 | `daily_purge_collected` | 闲鱼采集商品 `products`（`pinned_at IS NULL` 且无业务关联） | `xianyu_runs_retention_days` → 30 |
| 03:10 | `purge_pdd_search_runs` | PDD 采集流水 `pdd_search_runs` | `pdd_runs_retention_days` → 30 |
| 03:12 | `purge_xianyu_search_runs` | 闲鱼采集流水 `xianyu_search_runs` | `xianyu_runs_retention_days` → 30 |
| 03:14 | `purge_product_sightings` | 跨天同款观测 `product_sightings`（见 §10.4） | `sightings_retention_days` → 90 |

- **保留语义**：保留 N 天 = "到第 N 天才删"（cutoff = `now - N天`，删 `created_at < cutoff`），
  不是"只留今天"。改 N 立即对下一次清理生效。
- **保留：** 所有 `pinned_at` 非空的（人工 Pin，§10.2）——永不进任何清库。
- **前端可配**：「多平台比价」页 PDD / 闲鱼自动采集卡片的「流水保留天数」输入，
  写运行时配置（`pdd_worker_config.PARAM_SPECS` 的「数据清理」组）。
- **选品池/推荐池统一窗口**（commit `3f3d289`）：十维度选品候选词列表、两侧样本、
  套利推荐都改为读 N 天窗口（PDD 用 `pdd_runs_retention_days`、闲鱼商品保留 N 天后
  才清），不再每天 3 点归零。**去重逻辑不动**——仍是「每逻辑日每词一遍」，只放宽
  了"显示/选品池"的时间窗。
- `seen_count` 累积仍保留，作为人工 Pin 决策参考信号，但不触发自动 Pin。

### 10.4 跨天同款识别 Phase 1（精确指纹，2026-06-06 上线）

> 目的：N 天保留后同一商品可能多天被采到，要能认出"这是同一个"，做"出现 N 天 /
> 持续在售"标签 + 价格/热度跨天趋势，作为更稳的爆款信号。Phase 1 只做**精确指纹**
> （同一挂牌/快照），模糊/语义同款留作 Phase 2。

- **表 `product_sightings`**（migration `product_sighting_01`，模型 `ProductSighting`）：
  `platform / item_key / seen_date / keyword / title / price / heat / image_url`，
  `UNIQUE(item_key, seen_date)` 保证"每商品每逻辑日一条"（同日多次采到走 upsert）。
- **item_key（稳定指纹）**：闲鱼 `xy:<source_id>`；PDD `pdd:<sha1(clean_title)>`
  （列表页拿不到 goods_id，先用清洗后标题哈希；§11.2 详情页 goods_id 落地后可升级为
  `pdd:<goods_id>` 更精确）。服务见 `app/services/selection/sightings.py`。
- **写入点**：闲鱼 `_instant_search` upsert products 后、PDD `persist_pdd_result` 落
  run 后各记一条当日观测（best-effort，失败不连累采集主流程）。
- **消费**：`api/v1/selection.py::_attach_sighting_stats` 在十维度分析时按 item_key
  聚合 `first_seen / last_seen / days_seen / price_history` 富化进 payload；前端
  `TenDimSelection.tsx` 样本表「动态」列展示「出现 N 天」标签 + 价格 mini sparkline +
  历史 tooltip。
- **保留**：90 天（比流水长，专为看长期趋势；`sightings_retention_days` 可调）。

### 10.5 候选词手动移除（2026-06-06）

十维度选品候选词列表每行最右「×」直接删除（无确认）：清该词的 `selection_analysis`
分析缓存 + 全部 `pdd_search_runs` 流水 + 闲鱼采集商品（**保留 Pin**：`pinned_at` 非空
不删；跨天观测 `product_sightings` 不动）。行随即消失，下次再采到会重现。
接口 `DELETE /api/v1/selection/ten-dim/{keyword}`（commit `0f01bf9`/`89b47e1`）。

## 11. 查询模式：快速 vs 深度

> **2026-05-31 更新**：模式简化为两档「快速 / 深度」（早期下拉里的 `detail_smart`
> / `detail_deep` 占位项已下线）。深度模式的策略也做了修正——**不再"每条都点进
> 详情页"**（那样进详情页次数太多、风控暴露面大），改为**只挑信号强的少数几条进
> 详情页**。

| | **快速**（默认） | **深度** |
|---|---|---|
| 底层 `pdd_mode` 值 | `fast` | `list_deep`（沿用旧值，不迁移 DB） |
| 操作链路 | 仅列表页 | 列表页 + **选择性**点进详情页（只进 Top-K 条） |
| 单任务耗时 | 15-30s | 60-150s（取决于 K） |
| 列表页字段 | title / 价格 / 图片 / 销量 / 百亿补贴标 | 同左 |
| 详情页补抓字段 | — | **`goods_id` + 唤起链接** / SKU / 评论数 / 历史价 / 店铺评分 / 完整描述 / 百亿补贴真实价(OCR) |
| 单号日上限 | 60-80 次 | 15-25 次 |
| 适用场景 | 选品自动发现、即时搜索看大概 | 已圈定的"候选品"做完整评分 |

UI 设计：词库每词一列「采集模式」下拉（默认快速）。即时搜索/单品「升级为深度查询」按钮留作后续。

### 11.1 实现现状（2026-05-31）

| 能力 | 状态 |
|---|---|
| 快速（仅列表页，滚到够数即停） | ✅ 已上线 |
| 深度 = 多滚屏深扫（`list_deep` → deep / target 30 / 5 屏） | ✅ 已上线（**暂未进详情页**） |
| 深度「选择性进详情页」+ 抓 `goods_id` | ⬜ Phase 2 待实现（见 §11.2） |
| 找回真实商品（按需单条升级 / 人工匹配）+ 唤起链接 | ⬜ 待实现（见 §11.3） |
| 「查物流」拟人行为档位 | ✅ v1 已上线（A burst 结尾 + B 静默期双触发/各自概率 + 每日探测/冷却 + OCR 兜底 selector + 前端开关；任务记录可见；真机 2026-06-05 验证通过；见 §11.4） |
| 数据保留改为「按 N 天」+ 逻辑日界 3 点对齐 | ✅ 已上线（PDD/闲鱼流水 + 闲鱼商品 + 跨天观测各自 N 天可配；选品池/推荐池统一窗口；见 §10.3） |
| 跨天同款识别 Phase 1（精确指纹 + 出现N天/价格趋势） | ✅ 已上线（`product_sightings` + 十维选品「动态」列；见 §10.4） |
| 候选词手动移除（×直接删该词选品数据） | ✅ 已上线（见 §10.5） |
| 自动下单模块 | ❌ 不做，下单一律手动（见 §11.5） |

> 即：当前选「深度」拿到的是"多滚几屏、抓更多列表项"，**还没有进详情页**。
> 详情页补抓字段（SKU / 评论 / 历史价 / OCR 百亿补贴价）要等 §11.2 落地。

### 11.2 深度模式「选择性进详情页」设计（Phase 2 待实现）

**目标**：在深度模式下，先抓列表页拿到每条的标签(badges)/销量/价格，然后**按信号强度排序，只对 Top-K 条进详情页补抓**，兼顾数据完整度与风控暴露面。

1. **候选打分**：列表抓完后，对每条按 `标签数(badges) + 销量` 排序（销量缺失记 0；
   百亿补贴/品牌等标签权重可后调）。取分最高的 **Top-K**（K 很小，建议 3~5，
   出于风控；具体值实现时再定，可做成配置）。
2. **进详情页补抓**：对 Top-K 逐条点进商品详情页，抓 **`goods_id`（最重要，见 §11.3）** /
   SKU / 评论数 / 历史价 / 店铺评分 / 完整描述；**百亿补贴真实价仍要走 OCR**（canvas 自绘，u2 抓不到，见 Day 4）。
   - ⚠️ **待真机调研**：`goods_id` 怎么从 APP 详情页可靠抠出来未定——可能在详情页的
     activity intent / 页面元素里，也可能要点一下"分享"才暴露。实现前先真机验证，别写死方案。
3. **拟人节奏**：进详情页之间走人类化操作（停留 1.5-2.8s、60% 概率滑图看详情、
   30% 嵌套二次滑、随机回退），**复用 `_idle_browse_warmup` / 详情页 warmup 那套**，
   避免"详情页一动不动"的机器人特征（见 §Day 3.5 经验）。
4. **配额联动**：深度任务计入更高的"成本权重"，单号日上限按 15~25 次控制；
   `daily_search_quota` 消耗可对深度任务按 K 折算（实现时定）。
5. **落库**：详情页字段并入 `PddSearchRun` / 商品结果结构，喂给十维度评分的
   维度 4（SKU/规格）& 维度 9（口碑/评论）——见 §3.1。
6. **接线点**：`backend/app/services/pdd_autobatch.py` 的 `MODE_MAP` 里 `list_deep`
   仍走列表深扫；新增详情阶段在 worker `pdd_app_client.py` 实现（列表抓完 →
   排序选 Top-K → 逐条进详情页 → 回列表），由 payload 增加 `detail_top_k` 等参数驱动。

### 11.3 找回真实商品 & 唤起链接（2026-06-02 讨论定）

**背景**：列表页卡片是原生 view，控件树拿不到 `goods_id` / 链接，所以**只有进过详情页的商品才有唯一标识**。落库现状每条只有：完整标题(content-desc 全量) / 价格 / 销量 / 标签 / 缩略图。

**找回真实商品的两条路（互补）**：

- **A. 按需"单条升级为详情"（最准，推荐）**：选品页每行加「找原商品 / 抓链接」按钮。
  点一下 → worker 拿这条**完整标题**重新搜 → 结果里按 `标题 + 价格(+缩略图)` 匹配那张卡
  → 进详情页抓 `goods_id` → 拼唤起链接返回。只在你真要找某条时触发，频率极低，复用
  深度模式的详情页机器。**没有 goods_id 就没有唯一键，这是唯一能"精确"找回的程序化办法。**
- **B. 人工视觉匹配（0 额外开发）**：UI 给「复制完整标题」按钮 + 展示价格 + 缩略图。
  人拿标题去 PDD 搜，用图片+价格在首屏肉眼对。多数能对上，但同款多/排序变，不保证 100%。

> 日常用 B，要精准用 A。两者都**不需要把所有卡片都进详情页**，符合风控克制原则。

**唤起链接**：

- **风险不在链接，在"谁点"**：**人**在手机上点链接打开 PDD 商品 = 等同点开别人分享的链接，
  正常真人行为，**基本零风险**。**绝不让 worker 自动批量唤起**（那是自动化指纹）。唤起链接只供人点。
- **构造**：有 `goods_id` 后拼 `https://mobile.yangkeduo.com/goods.html?goods_id=XXX`（或 `pinduoduo://` scheme），
  装了 PDD 的手机点它直接跳进 APP 该商品。具体 URL 形态以真机分享出来的为准。

### 11.4 「查物流」拟人行为（2026-06-02 讨论定，2026-06-05 v1 上线）

**动机**：当前 `_idle_browse_warmup` 只会"看推荐流 / 看商品详情"，行为太单一（本质=搜了又搜+看商品）。
真人会查订单/物流，加这个提升**行为多样性**——正是 PDD 风控关注的维度。

**v1 实现（2026-06-05）**：

- **触发点（A + B 双触发，2026-06-05 真机验证后定稿）**：两个触发点并存，共用同一总开关 + 每日探测/冷却状态，**各自独立概率**（任一设 0 即关闭对应触发点）。
  - **A：burst 结尾**（`logistics_browse_prob`，默认 0.25）：`main._handle_search` 里 burst 结束（`burst_continues=False`）**且本次搜索没报错/没被风控**时，退 PDD 前调 `cli.maybe_browse_logistics()`（PDD 仍前台，便宜）。
  - **B：inter-burst 静默期中段**（`logistics_quiet_prob`，默认 0.35）：更像真人"两波搜索之间点亮手机看眼快递"。`BurstScheduler._quiet_sleep_with_break` 把整段静默 sleep 切成「睡一会 → 插一次查物流 → 睡剩下」，插入点在静默 [25%, 75%] 区间随机（避免固定相位指纹），插入动作耗时从剩余里扣掉、保证总静默 ≈ 目标 gap（不破坏 burst 节奏画像）。静默 < 90s 时退化为整段 sleep。回调 `main._quiet_logistics_break` 先调**廉价门控** `should_browse_logistics(prob=_LOGISTICS_QUIET_PROB)`（**不碰设备**），命中才前台化 PDD 跑 `cli.browse_logistics_now()`，结束 press home 退后台——避免"概率没中却白白前台化 PDD"。
  - 入口拆分：`should_browse_logistics(serial, prob=None)`（门控，prob 缺省=A 概率，B 显式传 quiet 概率）+ `browse_logistics_now()`（执行）；`maybe_browse_logistics()` = 两者组合（A 用）。
- **每日探测 + 冷却**（用户 2026-06-05 拍板）：状态机在 `pdd_app_client` 模块级维护（一个 worker 进程=一个采集号），`unknown / has_orders / cooldown` 三态，按东八区 day key 跨天自动重置。
  - 当天**首次触发**：进 个人中心 → 订单列表，识别订单页。**有真实订单** → 标 `has_orders`，当日后续触发继续随机查；**订单页为空** → 标 `cooldown`，当日不再尝试（空订单页反而是异常信号）；**导航没到订单页** → 保持 `unknown`，下次再试（不误判为冷却）。
- **路径**：回首页（补 `_ensure_home_tab`，静默期 PDD 后台/页面不定）→ 个人中心 → 待收货/查看全部（订单列表）→（识别到「查看物流/确认收货/申请退款/再次购买」等订单卡按钮=有单；**不认「评价」**，它同时是订单页顶部 tab 会误判）→ 点「查看物流」→ 停留 ~1.5-3s + 滑 1-2 下 → press back 2-3 次返回 → 由 cleanup 的 home press 收尾。轻量版（不逐个点开包裹）。
- **selector：OCR 兜底（2026-06-05 真机校准）**：PDD 把「个人中心/待收货/查看物流」等用 Canvas 自绘，`dump_hierarchy` 抓不到文字节点、xpath 点不到（真机连续两步卡死）。改为复用现成 EasyOCR：`ocr.locate_texts` 全屏/局部认字返回 bbox 中心绝对坐标，`pdd_app_client._tap_text` 先试 xpath（便宜）抓不到就 OCR 认字按坐标点。全链路（进个人中心/点订单入口/判有无单/点查看物流）均有 OCR 兜底，可传 `region_ratio` 只扫局部更快更准。底部「个人中心」另有"最右下角坐标"硬兜底。
- **拟人一致性**：滑动统一走 `_humanize_swipe_path`（曲线+缓动+起止点 x/y 抖动，对齐 `_idle_browse_warmup`）；OCR 点击按文字中心 ±10px 抖动（同 `_human_click` 不点正中心）；步骤间停顿走 `_sleep_jitter`、滑动/back 间隔走 `_pace_uniform`，**均受全局节奏因子 `_HUMANIZE_PACE`** 控制，与采集任务统一。
- **开关**：`pdd_worker_config` 加 `logistics_browse_enabled`（总开关，默认 **False**）/ `logistics_browse_prob`（A·burst 结尾，默认 0.25）/ `logistics_quiet_prob`（B·静默期，默认 0.35），走 runtime-config 热更新（`apply_remote_config → set_logistics_browse`，≤45s 生效）。前端「多平台比价」页 PDD 全自动采集卡片里有「查物流」总开关 +「结尾概率」+「静默概率」两个输入。
- **任务记录可见（2026-06-05）**：每次查快递执行后 worker 经 `POST /pdd-worker/logistics` 上报落 `logistics_runs` 表（独立表，不污染 PDD/闲鱼采集统计），「任务记录」抽屉合并展示。行字段：平台=`查快递`、来源=触发点(A 结尾/B 静默)、状态=`viewed`(有单已查物流)/`empty`(订单页空)/`nav_failed`(未进订单页)、采集号、耗时。可按 `platform=logistics` 单独筛选。上报 best-effort，丢失不影响采集主流程。
- **冒烟**：`python -m pdd_app_worker.smoke_logistics`（绕过开关/概率/冷却，强制跑一遍 `_browse_logistics_once`，肉眼校准 + 不占队列）。2026-06-05 真机 J7JJOVHQYHU8HMHE 实测通过（OCR 命中 个人中心/查看全部/查看物流，result=True）。
- **⚠️ 前提**：该号要**真有历史订单**——没有就会命中"当日冷却"，开了也不会乱逛空订单页。建议只对有真实购买记录的号开。EasyOCR 模型需已预下到 `~/.EasyOCR/model/`（与价格识别共用）。

### 11.5 真实下单策略（2026-06-02 决定）

**结论：下单一律手动，不开发自动下单模块。**

- 量级是"偶尔选一两个验证"，手动远比开发划算。
- 支付环节（密码/指纹/风控）基本无法自动化，且**下单/支付是 PDD 风控最敏感动作**，自动化极易触发、还会**连累采集号**（养号前功尽弃）。
- 结算页（SKU/地址/优惠券/拼单）改版频繁，维护成本高、收益低。
- 系统只负责"选出值得买的"，把候选（标题+图+价+§11.3 的唤起链接）推给人，人点链接跳进 APP 手动下单。最多半自动到"帮你跳到商品详情页"，付款永远留给人。
...
### 14.1.1 任务记录抽屉（2026-06-04 增补，轻量版）

工具栏「采集节奏」左侧新增「任务记录」按钮（`ProfileOutlined`）。点开抽屉，读 `GET /pdd-runs/`（**PDD + 闲鱼合并**，分页流水）展示**派发过的关键词搜索任务**：时间 / 平台 / 关键词 / 品类 / 采集号 / 来源（自动轮播·批量·手动·紧急）/ 模式（快速·深度）/ 状态 / 商品数 / 耗时；支持按平台、状态、来源过滤 + 分页。

- **PDD**：来自 `pdd_search_runs`（`list_runs`）。
- **闲鱼**（2026-06-04 补）：来自独立的 `xianyu_search_runs`（migration `xianyu_run_01`，模型 `XianyuSearchRun`）。为什么不复用 `pdd_search_runs`：那张表上的看板/配额/已采集池逻辑全是 PDD 语义，混入闲鱼会污染统计；单开一张表 PDD 侧零改动。`instant_search(platform='xianyu')` 跑完调 `persist_xianyu_run` 落一行（source：lib 自动 / batch 批量 / manual 手动，由调用方透传）。`GET /pdd-runs/` 用 `platform` 参数过滤，留空时两表各取前 N 条按 `created_at` 归并切片。
- **时间口径**：当前为**任务完成落库时刻**（`created_at`），对搜索任务≈派发后约 45s。不是精确派发时刻。闲鱼行无采集号/模式（guest 爬，列显示「—」）。
- **范围限制（待后续完整版 B 补）**：随机首页浏览（warmup）/ 查快递（§11.4 未实现）目前不是「派发的任务」、后端无记录，故不在列表内。完整版规划：新增「派发流水」落库（派发那一刻写一行，精确派发时间）+ worker 上报浏览/查快递事件，搜索+浏览+查快递按派发时间合并展示。
...
## 15. 多号品类绑定 + 队列按号路由（防双号画像趋同，2026-06-03 讨论定）

### 15.1 问题

多手机并发时，**所有号共用一个全局队列 `pdd_app:task_q`**（`pop_task` 不按号/品类过滤，谁先 BLPOP 谁拿），而 `dispatch_auto_batch` 每批只锁一个品类、一次性入队 N 个同品类词。结果：两个采集号从同一批同品类词里各抢几个、几乎同时搜，多日下来**搜的是同一个词库、同样的品类轮换、相近的时间** → 两号的**品类画像高度一致 + 行为时间高度相关**，像"同一操作者/机房批量跑"。

> 设备指纹（1 机 1 号绑死）和网络层（每号独立手机卡）**已隔离**，是最强信号；本节专门补齐"品类画像 + 时间相关性"这两项，落实 roadmap 既定的"1 号 1 主品类"原则。

### 15.2 设计

**号的来源**：`accounts` 表 `platform='pdd_crawler'` 的行（`account_name=pdd_crawler_XXXX`、`bound_device_serial` 绑手机，已有）。

**品类 ↔ 号：多对多**
- 一个品类可分配给 **1 个号（独占）** 或 **多个号（共用，如"生活类"）**。
- 新增关联表 `pdd_category_account`（`category_id` ↔ `account_id`）。
- **未分配给任何号的品类 = 不跑**（2026-06-03 决定：必须显式分配才采，最严格、避免无意中两号都跑）。

**派发改为按号**（`dispatch_auto_batch`）
- 对每个在线号，从**它被分配的品类**里挑最久没碰的一个品类、取该号今天还没跑的词，入**该号专属队列** `pdd_app:task_q:{account_name}`。
- 每号独立的随机 `next_ts`（已有 `pdd_app:auto_next_ts`，改为按号 key）→ 自然错峰，不会同一秒开搜。

**队列按号路由**
- 单一 `pdd_app:task_q` → 每号一条 `pdd_app:task_q:{account_name}`。
- worker `poll` 带上自己的 `BOUND_PDD_ACCOUNT`，后端只从该号队列 BLPOP；紧急插队/清队/队列深度都按号 key 操作。

**共用品类不撞同一个词**：现有"每词每天去重"（`done_pdd_keyword_ids`）已保证——共用品类里某词被号A今天跑了，号B不会再跑（天然负载均衡 + 不重复）。

**前端（词库管理页 `PddKeywords.tsx`）**
- 每个品类一个"分配给号"的多选控件（列出 `pdd_crawler` 号 + 其绑定手机），可勾 1 或多个号。
- 未分配的品类给明显提示（"未分配 → 不采集"）。

### 15.3 分阶段实现计划

- **Phase 1 ✅（后端骨架 + 前端分配 UI，commit `af75ba5`）**：
  1. 模型 + 迁移 `pdd_cat_acct_01`：`pdd_category_account` 关联表。
  2. API：`GET /pdd-keywords/accounts` 列 `pdd_crawler` 号；`categories` 带 `account_ids`；`PUT /pdd-keywords/categories/{id}/accounts` 整组覆盖分配。
  3. 前端词库管理页：选中品类后"分配给采集号"多选 + 未分配黄色告警 + 菜单⚠标记。
- **Phase 2 ✅（派发 + 队列按号路由）**：
  4. 队列 key 按号：`pdd_app:task_q:{account}`（`task_queue_key`）；`enqueue_task(account=)` / `pop_task(account=)`；`queue_depth`/`purge_queue` 聚合默认 + 所有按号队列。
  5. `select_cohesive_keywords(allowed_category_ids=)` 只在该号被分配品类里挑；`dispatch_auto_batch(account_name=, account_id=, allowed_category_ids=)` 入该号队列。
  6. `auto_next_ts` 按号（`pdd_app:auto_next_ts:{account}`）→ 各号独立随机错峰。
  7. 心跳带 `account` → `get_worker_status().accounts` 暴露在线号；`auto_batch_tick` 配过绑定就按在线号逐个派发，未配则走旧全局派发。
  8. `poll` 端点收 `account` query；worker `http_client` poll/心跳带 `BOUND_PDD_ACCOUNT`。

#### 路由语义 & 向后兼容
- **worker 取任务**：`BLPOP [自己的号队列, 默认队列]`——先吃自己被分配品类的自动跑批任务，再兜底吃默认队列里的手动派发任务，绝不会取到别的号的队列。
- **手动派发 / 紧急插队**（`pdd_runs` dispatch/batch）仍进**默认队列**（任一在线 worker 抢），不按号路由——保证手动操作即时性，emergency LPUSH 行为不变。
- **未配任何绑定**（`pdd_category_account` 空）→ `auto_batch_tick` 退回旧的全局派发到默认队列，不因漏配停采（线上平滑升级）。**一旦配了绑定**：未分配品类不再被自动跑批挑中（=不跑）。
- **闲鱼解耦**：多号模式下 PDD 派发不再带闲鱼（`both_platforms=False`），闲鱼由其独立 tick 驱动，避免按号重复触发。
- **旧 worker**（不传 `account`）只吃默认队列，行为与改造前一致。
- **已知小限制**：控制台"下次自动"时刻（`get_auto_next_ts()` 全局 key）在多号模式下为空（各号有独立 next_ts），仅展示退化，不影响采集。

- **Phase 3 ✅（控制台路由状态面板，commit `b7ea1be`）**：
  9. `/console` 返回 `routing`：每个 `pdd_crawler` 号的 在线/队列深度/分配品类数/下次派词时刻（`get_routing_status` + `account_queue_depth`）。
  10. 多平台比价页：配过绑定时顶部显示「多号路由状态」卡，肉眼确认双号各采各的品类 + 错峰。

### 15.4 上线与验证 checklist

**部署顺序**（schema 先于代码，附加表安全）：
1. ✅ **DB 迁移已上线**（2026-06-03，pod 内 `alembic upgrade head` → `pdd_cat_acct_01`，`pdd_category_account` 表已建）。
2. ⬜ **构建并部署新 backend + frontend 镜像**（含 Phase 1-3 代码）。在此之前 schema 已就位、旧镜像不受影响（多出一张空表而已）。
3. ⬜ **每台手机的 worker 配 `BOUND_PDD_ACCOUNT`**（见下「worker 账号绑定」）。
4. ⬜ **词库管理页分配品类给号**（共用类勾多个、独占类勾一个；未分配 = 不采集）。

**worker 账号绑定（每台手机一次性）**：
- `BOUND_PDD_ACCOUNT` 现在**既是审计也是路由键**：backend 按它把「分配给该号品类」的自动跑批任务发给这台机。
- 取值必须：①等于 `accounts` 表 `platform='pdd_crawler'` 的某个 `account_name`；②就是这台手机**此刻真正登录**的 PDD 号（不一致会采错品类、画像污染）。
- 现有号（2026-06-03）：`pdd_crawler_7315`（已绑手机1，serial `PKT0220416005274`）、`pdd_crawler_1876/2117/4310/5514`（未绑，备用）。
- 配置位置：`worker/start_phone_N.bat` 里 `set BOUND_PDD_ACCOUNT=...`（同时设唯一 `ADB_SERIAL` + `WORKER_NAME`）。
- 换号：跑 `backend/scripts/pdd_account_swap.py` 把 `accounts.bound_device_serial` 迁到新号，并同步改 `.bat` 里的 `BOUND_PDD_ACCOUNT`。

**验证（部署后）**：
- worker 启动日志应出现 `BOUND_PDD_ACCOUNT=pdd_crawler_xxxx`，且该值非 `unknown`。
- 多平台比价页「多号路由状态」卡：每个在线号显示「在线 · serial · 分配 N 品类」；两个号「下次派词」时刻不同 = 错峰生效。
- 跑一阵后看「已采集池」/`pdd_search_runs.account_name`：同一品类的词应集中在被分配的那个号名下，不会两个号都出现同一品类。
- 想确认隔离彻底：给号A只分配品类X、号B只分配品类Y，跑半天后两号的 `account_name × category` 应无交叉。

### 15.5 worker 部署运维：免冲突本地配置 + 镜像拉取（2026-06-03 增补）

起因：home worker `git pull` 总卡——①直连 github.com 国内常 `Connection reset`/443 超时；②`start_phone_N.bat` 里手填的 serial/账号是本地改动，和仓库更新撞文件导致 pull 中止。两个根因各给一个一劳永逸方案：

**① 机器专属配置挪出 git（serial/采集号）**
- `start_phone_N.bat`（已纳管）只保留通用逻辑 + `WORKER_NAME=phone-N`，机器专属的 `ADB_SERIAL` + `BOUND_PDD_ACCOUNT` 改为 `call` 同目录下的 `phone_N.env.local.bat`。
- `phone_N.env.local.bat` 被 `.gitignore` 忽略（`worker/*.env.local.bat`）→ `git pull` 永不覆盖。
- 模板：`worker/phone.env.example.bat`，复制成 `phone_N.env.local.bat` 填真实值即可。
- `.bat` 找不到本地文件会**报错退出**（fail loud），不会静默用错号。
- **一次性迁移（每台 worker PC，部署本次更新后做）**：
  ```bat
  cd C:\resell
  git checkout -- worker/start_phone_1.bat worker/start_phone_2.bat   REM 丢掉旧的本地手填值
  worker\pull-latest.bat                                              REM 走镜像拉新代码
  copy worker\phone.env.example.bat worker\phone_1.env.local.bat      REM 填 PKT0220416005274 + pdd_crawler_7315
  copy worker\phone.env.example.bat worker\phone_2.env.local.bat      REM 填 J7JJOVHQYHU8HMHE + pdd_crawler_1876
  ```
  之后再 `git pull` 不会再碰本地文件，重启 `start_phone_N.bat` 即可。

**② 镜像拉取脚本**
- `worker/pull-latest.bat`：走 GitHub 镜像（默认 `https://ghfast.top`）`git pull`，避开直连失败。镜像失效就改脚本里 `MIRROR=`（备选 `gh-proxy.com` / `mirror.ghproxy.com` / `ghproxy.net`）。

### 15.6 SOP：新增一台手机 / 一个采集号

> 现有号：`pdd_crawler_7315`(手机1, `PKT0220416005274`) / `pdd_crawler_1876`(手机2, `J7JJOVHQYHU8HMHE`) / `_2117` / `_4310` / `_5514`（后三个已在 `accounts` 表、未绑机，可直接拿来用）。

**第 1 步（手机上）** 在新手机的 PDD APP 里登录要用的号，确保停在能正常搜索的状态。

**第 2 步（worker PC）** 插上手机、开 USB 调试，`adb devices` 取序列号（要 `device` 状态那串）。

**第 3 步（DB 绑定，需能连集群 DB → 由后端侧/有 kubectl 的人做，家里 worker PC 连不到内网 DB）**
- 复用备用号（已在 accounts 表，如 `pdd_crawler_2117`）→ 在 postgres pod 跑：
  ```bash
  kubectl exec resell-manager-postgresql-0 -c postgresql -- sh -c 'PGPASSWORD=$POSTGRES_PASSWORD psql -U postgres -d postgres <<SQL
  BEGIN;
  UPDATE accounts SET bound_device_serial=NULL, updated_at=NOW()
   WHERE platform='"'"'pdd_crawler'"'"' AND bound_device_serial='"'"'<SERIAL>'"'"' AND account_name<>'"'"'pdd_crawler_<尾号>'"'"';
  UPDATE accounts SET is_active=true, session_status='"'"'none'"'"', lifecycle_stage='"'"'nurturing'"'"',
         bound_device_serial='"'"'<SERIAL>'"'"', suspended_reason=NULL, cooldown_until=NULL, updated_at=NOW()
   WHERE platform='"'"'pdd_crawler'"'"' AND account_name='"'"'pdd_crawler_<尾号>'"'"';
  COMMIT;
  SQL'
  ```
- 全新号（accounts 表里没有）→ 把上面第二条 `UPDATE` 换成 `INSERT`，或在能连 DB 的机器上跑 `backend/scripts/pdd_account_swap.py`（它会 upsert + quarantine 旧号；注意它要 `asyncpg` + DATABASE_URL 可达，家里 worker PC 跑不了）。
- 一机一号铁律：第一条 `UPDATE` 会解绑任何误绑到该 serial 的其它号。

**第 4 步（worker PC）** 建机器专属配置（不进 git）：
```bat
cd C:\resell\worker
notepad phone_N.env.local.bat     REM N=3/4...；点“是”新建，填两行后保存
```
内容：
```bat
set ADB_SERIAL=<SERIAL>
set BOUND_PDD_ACCOUNT=pdd_crawler_<尾号>
```

**第 5 步** 双击 `start_phone_N.bat` 启动（缺 `phone_N.env.local.bat` 会报错退出）。启动日志应显示 `BOUND_PDD_ACCOUNT=pdd_crawler_<尾号>`。

**第 6 步（验证 + 分配）**
- 查在线号：`get_worker_status().accounts` 应含新号（或多平台比价页「多号路由状态」卡出现该号）。
