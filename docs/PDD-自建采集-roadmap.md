...
- **Phase 1（后端骨架 + 前端分配 UI）**：
  1. 模型 + 迁移：`pdd_category_account` 关联表。
  2. API：列 `pdd_crawler` 号（带 `bound_device_serial`）；品类↔号 的查询/分配。
  3. 前端：词库管理页加"分配给号"多选 + 未分配提示。
- **Phase 2（派发 + 队列按号路由）**：
  4. `dispatch_auto_batch` 改为按号挑词（只挑该号被分配品类的词）、入 per-account 队列；`auto_next_ts` 改按号。
  5. 队列 key 按号（enqueue/pop/purge/depth/紧急插队）；`poll` 端点按 `BOUND_PDD_ACCOUNT` 取。
  6. worker `poll` 带 account 身份。
- **向后兼容**：未配 account 的旧 worker / 单号场景退化到默认队列，不报错。
