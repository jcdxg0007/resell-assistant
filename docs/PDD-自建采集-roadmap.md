...
工具栏「采集节奏」左侧新增「任务记录」按钮（`ProfileOutlined`）。点开抽屉，读现有 `GET /pdd-runs/`（`list_runs`，分页流水）展示**派发过的关键词搜索任务**：时间 / 关键词 / 品类 / 采集号 / 来源（自动轮播·批量·手动·紧急）/ 模式（快速·深度）/ 状态 / 商品数 / 耗时；支持按状态、来源过滤 + 分页。

- **时间口径**：当前为**任务完成落库时刻**（`pdd_search_runs.created_at`），对搜索任务≈派发后约 45s。不是精确派发时刻。
