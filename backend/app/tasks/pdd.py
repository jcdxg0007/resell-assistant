...
from app.services.pdd_autobatch import await_and_persist_one, dispatch_auto_batch
...
async def _auto_batch_tick():
    ...
    # 通过时间闸门 → 先排下一次随机时刻（避免本轮还没派完又被触发），再派
    ...
    if not (await get_worker_status()).get("online"):
